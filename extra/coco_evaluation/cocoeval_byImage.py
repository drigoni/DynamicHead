...
class COCOeval:
    ...
    def accumulate(self, p = None):
        ...
        M           = len(p.maxDets)
        precision   = -np.ones((T,R,I,M)) # -1 for the precision of absent categories
        recall      = -np.ones((T,I,M))
        scores      = -np.ones((T,R,I,M))
            ...
            for m, maxDet in enumerate(m_list):
                ...
                for t, (tp, fp) in enumerate(zip(tp_sum, fp_sum)):
                    ...
                    try:
                        ...
                        pass
                    precision[t,:,i,m] = np.array(q)
                    scores[t,:,i,m] = np.array(ss)
        self.eval = {
            'params': p,
            'counts': [T, R, I, M],
