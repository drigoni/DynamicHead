...
class COCOeval:
    ...
    def accumulate(self, p = None):
        ...
        A           = len(p.areaRng)
        M           = len(p.maxDets)
        precision   = -np.ones((T,R,I,A,M)) # -1 for the precision of absent categories
        recall      = -np.ones((T,I,A,M))
        ...
        for image_index, _ in enumerate(i_list):
            ...
            for a, a0 in enumerate(a_list):
                ...
                for m, maxDet in enumerate(m_list):
                    E = [self.evalImgs[Ni + Na + k] for k in k_list]
                    E = [e for e in E if not e is None]
                    # Only when there are no GT for images: 576 images in the validation
                    if len(E) == 0:
                        n_exit1 = n_exit1 + 1
                        continue
                    dtScores = np.concatenate([e['dtScores'][0:maxDet] for e in E])                 # [n_dt * maxDet (al massimo)]

                    # different sorting method generates slightly different results.
                    # mergesort is used to be consistent as Matlab implementation.
                    inds = np.argsort(-dtScores, kind='mergesort')                                  # [n_dt * maxDet (al massimo)]
                    dtScoresSorted = dtScores[inds]                                                 # [n_dt * maxDet (al massimo)]

                    dtm  = np.concatenate([e['dtMatches'][:,0:maxDet] for e in E], axis=1)[:,inds]  # [T=10, n_dt * maxDet (al massimo)]
                    dtIg = np.concatenate([e['dtIgnore'][:,0:maxDet]  for e in E], axis=1)[:,inds]  # [T=10, n_dt * maxDet (al massimo)]
                    ...
                    for t, (tp, fp) in enumerate(zip(tp_sum, fp_sum)):
                        ...
                        # make the plot smooth. The value before should always be bigger than the one after in order to avoid numerical oscillations in the final evaluation
        ...
        for i, i0 in enumerate(i_list):
            ...
            if len(subset[subset>-1]) == 0:
                ...
                mean_s = np.mean(subset[subset>-1])
    ...
    def summarize(self):
        ...
        def _summarizeDets():
            stats = np.zeros((12,))
            stats[0] = _summarize(1, iouThr=None, areaRng='all', maxDets=self.params.maxDets[2])
            stats[1] = _summarize(1, iouThr=.5, areaRng='all', maxDets=self.params.maxDets[2])
