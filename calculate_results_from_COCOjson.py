...
# from extra.coco_evaluation.cocoeval import COCOeval
...
class COCOEvaluator(DatasetEvaluator):
        ...
        *,
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            with open(json_file, 'r') as f:
                self.image_concepts = json.load(f)['images']
    ...
    def evaluate(self, file_path):
        ...
        if self.filtering:
            ...
            predictions = filtering_process(self._coco_api, predictions, self.coco2synset, self._metadata)
...
def parse_args():
    ...
    parser.add_argument('--file', dest='file',
                        help='Results.',
                        default="./results/COCO/dh/dh_r50_fpn_COCO/inference/instances_predictions.pth",
                        type=str)
    ...
    parser.add_argument('--filtering', dest='filtering',
                        help='Filtering process.',
                        default='True',
                        type=lambda x: True if x.lower() == 'true' else False)
...
if __name__ == "__main__":
    args = parse_args()
    print(args)
    # COCOEvaluator(args.dataset,
    # concept_finder = ConceptFinder(args.coco2concepts, depth=args.level, unique=args.unique, only_name=True)
    concept_finder = ConceptFinder(args.coco2concepts, depth=args.level, unique=args.unique, only_name=True)
    coco2synset = concept_finder.coco2synset

    evaluator = COCOEvaluator(args.dataset,
                coco2synset=coco2synset, 
                tasks=None,
                distributed=False,
                output_dir='./results/',
                max_dets_per_image=None,
